...
//  cout <<CYAN<<"  256 "<<"  2015-2016"<<"  1   "<<"  11门    "<<"  27  "<<"  4.35    "<<"  4.55      "
    ...
    for(int i=EntranceYear;i<=CurrentTime.getYear();i++)
    {
        ...
        {
        int TotalRank=getNumberOfTerm(i,j);
        cout<<left<<setfill(' ')<<setw(6)<<TotalRank<<setw(11)<<i<<"-"<<i+1<<setw(6)<<j<<setw(10)<<Terms[TotalRank].getNumberOfLessonRefer();
        cout<<setw(6)<<Terms[TotalRank].getCredit()<<setw(10)<<Terms[TotalRank].getAverage()<<setw(12)<<Terms[TotalRank].getAverageExpectPubEle()<<RESET<<endl;
        }
    }
    return 0;
}
